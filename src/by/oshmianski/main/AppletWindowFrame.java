...
public class AppletWindowFrame extends JFrame {
    ...
    public AppletWindowFrame(String server) {
        ...
        ap.setServer(server);
        ap.setDbIDProc(props.getProperty("dbIDProc"));
        ap.setDbIDLiK(props.getProperty("dbIDLiK"));
        ap.setViewSuit(props.getProperty("viewSuit"));
        ap.setViewDebt(props.getProperty("viewDebt"));
        ...
        setTitle("Фильтратор");
        setIconImage(Icons.iconToImage(IconContainer.getInstance().loadImage("filter.png")));
...
//                    System.exit();
        ...
        LoadReestrs loadReestrs = new LoadReestrs(gui.getDockingContainer().getUIProcessor());
        loadReestrs.execute();
    }
